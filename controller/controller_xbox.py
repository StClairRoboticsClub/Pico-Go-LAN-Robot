...
class XboxController:
    ...
    def _initialize_controller(self):
        """Initialize the specified joystick."""
        joystick_count = pygame.joystick.get_count()
        
        if joystick_count == 0:
            return  # Silent - will fall back to keyboard
        
        # Validate joystick index
        if self.joystick_index < 0 or self.joystick_index >= joystick_count:
            return  # Invalid index - will fall back to keyboard
    ...
    def get_joystick_index(self) -> int:
        """Get the joystick index this controller is using."""
        return self.joystick_index
...
class ControllerApp:
    ...
    def __init__(self, robot_ip: str, joystick_index: int = 0):
        """
        ...
        # Update display with controller type and gamepad info
        if self.controller_type == "Xbox":
            gamepad_label = f"Gamepad {joystick_index}"
            self.display.update(controller_type=f"{self.controller_type} ({gamepad_label})", robot_ip=robot_ip)
        else:
...
if __name__ == "__main__":
    ...
    # It should be launched via: python3 controller_xbox.py <robot_ip>
    # Always use the Master GUI to launch controllers!
    import sys
    if len(sys.argv) < 2:
